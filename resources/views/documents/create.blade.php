...
@section('content')
    ...
    <h1 class="text-2xl font-semibold mb-6">New document</h1>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="type" id="type" class="w-full rounded-md border-gray-300 shadow-sm text-sm">
                    <option value="devis" {{ old('type') == 'devis' ? 'selected' : '' }}>Quote</option>
                    <option value="facture" {{ old('type') == 'facture' ? 'selected' : '' }}>Invoice</option>
                </select>
                @error('type')
        ...
        <div>
            <div class="flex items-center justify-between mb-2">
                ...
                <button type="button" onclick="ajouterLigne()"
                        class="text-sm text-blue-600 hover:underline">+ Add a line</button>
            </div>

            <div id="lignes-container" class="space-y-2">
                {{-- Lines are injected here via JS --}}
            </div>
        </div>
...
<script>
    ...
    function ajouterLigne() {
        ...
        div.innerHTML = `
            <input type="text" name="lignes[${ligneIndex}][designation]" placeholder="Description"
                   class="col-span-5 rounded-md border-gray-300 shadow-sm text-sm">
            <input type="number" name="lignes[${ligneIndex}][quantite]" placeholder="Qty" step="0.01" value="1"
                   class="col-span-2 rounded-md border-gray-300 shadow-sm text-sm">
            <input type="number" name="lignes[${ligneIndex}][prix_unitaire]" placeholder="Unit price" step="0.01"
                   class="col-span-2 rounded-md border-gray-300 shadow-sm text-sm">
            <input type="number" name="lignes[${ligneIndex}][taux_tva]" placeholder="VAT %" step="0.01" value="20"
                   class="col-span-2 rounded-md border-gray-300 shadow-sm text-sm">
